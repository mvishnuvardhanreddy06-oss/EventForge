import React from 'react';
import { Search, X, CalendarRange, Calendar, List, SlidersHorizontal } from 'lucide-react';

const SessionToolbar = ({
  searchQuery,
  onSearchChange,
  sessionTypeFilter,
  onSessionTypeChange,
  statusFilter,
  onStatusChange,
  roomFilter,
  onRoomChange,
  speakerFilter,
  onSpeakerChange,
  dateFilter,
  onDateChange,
  onClearFilters,
  hasActiveFilters,
  viewMode,
  onViewModeChange,
  totalResults,
  rooms = [],
  speakers = []
}) => {
  return (
    <div className="bg-white rounded-2xl border border-slate-200/80 p-3.5 shadow-2xs space-y-3">
      <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-3">
        {/* Search Input */}
        <div className="relative flex-1 min-w-[260px]">
          <Search className="w-4 h-4 text-slate-400 absolute left-3.5 top-1/2 -translate-y-1/2 pointer-events-none" />
          <input
            type="text"
            value={searchQuery}
            onChange={(e) => onSearchChange(e.target.value)}
            placeholder="Search by session title, speaker or room..."
            className="w-full pl-9 pr-9 py-2 text-xs text-slate-900 bg-slate-50/70 border border-slate-200 rounded-xl placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-600/20 focus:border-blue-600 transition-all"
          />
          {searchQuery && (
            <button
              onClick={() => onSearchChange('')}
              className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 p-0.5 rounded-md hover:bg-slate-200/60 transition-colors"
              title="Clear search"
            >
              <X className="w-3.5 h-3.5" />
            </button>
          )}
        </div>

        {/* Filters and View Mode Switcher */}
        <div className="flex flex-wrap items-center gap-2">
          {/* Session Type Filter */}
          <div className="relative">
            <select
              value={sessionTypeFilter}
              onChange={(e) => onSessionTypeChange(e.target.value)}
              className="appearance-none pl-3 pr-8 py-2 text-xs font-semibold text-slate-700 bg-slate-50/70 border border-slate-200 rounded-xl hover:bg-slate-100/60 focus:outline-none focus:ring-2 focus:ring-blue-600/20 focus:border-blue-600 transition-all cursor-pointer"
            >
              <option value="all">All Types</option>
              <option value="Keynote">Keynote</option>
              <option value="Workshop">Workshop</option>
              <option value="Panel">Panel</option>
              <option value="Talk">Talk</option>
              <option value="Networking">Networking</option>
              <option value="Break">Break</option>
            </select>
            <span className="pointer-events-none absolute right-2.5 top-1/2 -translate-y-1/2 text-[10px] text-slate-400">
              ▼
            </span>
          </div>

          {/* Status Filter */}
          <div className="relative">
            <select
              value={statusFilter}
              onChange={(e) => onStatusChange(e.target.value)}
              className="appearance-none pl-3 pr-8 py-2 text-xs font-semibold text-slate-700 bg-slate-50/70 border border-slate-200 rounded-xl hover:bg-slate-100/60 focus:outline-none focus:ring-2 focus:ring-blue-600/20 focus:border-blue-600 transition-all cursor-pointer"
            >
              <option value="all">All Status</option>
              <option value="scheduled">Scheduled</option>
              <option value="draft">Draft</option>
              <option value="live">Live</option>
              <option value="completed">Completed</option>
              <option value="cancelled">Cancelled</option>
            </select>
            <span className="pointer-events-none absolute right-2.5 top-1/2 -translate-y-1/2 text-[10px] text-slate-400">
              ▼
            </span>
          </div>

          {/* Room Filter */}
          <div className="relative">
            <select
              value={roomFilter}
              onChange={(e) => onRoomChange(e.target.value)}
              className="appearance-none pl-3 pr-8 py-2 text-xs font-semibold text-slate-700 bg-slate-50/70 border border-slate-200 rounded-xl hover:bg-slate-100/60 focus:outline-none focus:ring-2 focus:ring-blue-600/20 focus:border-blue-600 transition-all cursor-pointer"
            >
              <option value="all">All Rooms</option>
              <option value="Main Hall">Main Hall</option>
              <option value="Hall A">Hall A</option>
              <option value="Hall B">Hall B</option>
              <option value="Room 101">Room 101</option>
              <option value="Room 201">Room 201</option>
              {rooms.map((r, i) => (
                <option key={i} value={r.name || r}>
                  {r.name || r}
                </option>
              ))}
            </select>
            <span className="pointer-events-none absolute right-2.5 top-1/2 -translate-y-1/2 text-[10px] text-slate-400">
              ▼
            </span>
          </div>

          {/* Speaker Filter */}
          <div className="relative">
            <select
              value={speakerFilter}
              onChange={(e) => onSpeakerChange(e.target.value)}
              className="appearance-none pl-3 pr-8 py-2 text-xs font-semibold text-slate-700 bg-slate-50/70 border border-slate-200 rounded-xl hover:bg-slate-100/60 focus:outline-none focus:ring-2 focus:ring-blue-600/20 focus:border-blue-600 transition-all cursor-pointer"
            >
              <option value="all">All Speakers</option>
              {speakers.map((sp, i) => (
                <option key={i} value={sp.name || sp}>
                  {sp.name || sp}
                </option>
              ))}
            </select>
            <span className="pointer-events-none absolute right-2.5 top-1/2 -translate-y-1/2 text-[10px] text-slate-400">
              ▼
            </span>
          </div>

          {/* Date Filter */}
          <div className="relative">
            <select
              value={dateFilter}
              onChange={(e) => onDateChange(e.target.value)}
              className="appearance-none pl-3 pr-8 py-2 text-xs font-semibold text-slate-700 bg-slate-50/70 border border-slate-200 rounded-xl hover:bg-slate-100/60 focus:outline-none focus:ring-2 focus:ring-blue-600/20 focus:border-blue-600 transition-all cursor-pointer"
            >
              <option value="all">All Dates</option>
              <option value="Sep 24, 2026">Sep 24, 2026 (Today)</option>
              <option value="Sep 25, 2026">Sep 25, 2026 (Tomorrow)</option>
            </select>
            <span className="pointer-events-none absolute right-2.5 top-1/2 -translate-y-1/2 text-[10px] text-slate-400">
              ▼
            </span>
          </div>

          {/* Clear Filters Button */}
          {hasActiveFilters && (
            <button
              onClick={onClearFilters}
              className="inline-flex items-center space-x-1.5 px-3 py-2 text-xs font-bold text-rose-600 bg-rose-50 hover:bg-rose-100/80 border border-rose-200/80 rounded-xl transition-colors"
            >
              <X className="w-3.5 h-3.5" />
              <span>Clear Filters</span>
            </button>
          )}

          {/* View Switcher: Agenda | Calendar | List */}
          <div className="flex items-center bg-slate-100 p-0.5 rounded-xl border border-slate-200/70 ml-auto">
            <button
              type="button"
              onClick={() => onViewModeChange('agenda')}
              className={`inline-flex items-center space-x-1 px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${
                viewMode === 'agenda'
                  ? 'bg-white text-blue-600 shadow-2xs'
                  : 'text-slate-500 hover:text-slate-800'
              }`}
              title="Agenda Timeline"
            >
              <CalendarRange className="w-3.5 h-3.5" />
              <span className="hidden sm:inline">Agenda</span>
            </button>
            <button
              type="button"
              onClick={() => onViewModeChange('calendar')}
              className={`inline-flex items-center space-x-1 px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${
                viewMode === 'calendar'
                  ? 'bg-white text-blue-600 shadow-2xs'
                  : 'text-slate-500 hover:text-slate-800'
              }`}
              title="Calendar Schedule"
            >
              <Calendar className="w-3.5 h-3.5" />
              <span className="hidden sm:inline">Calendar</span>
            </button>
            <button
              type="button"
              onClick={() => onViewModeChange('list')}
              className={`inline-flex items-center space-x-1 px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${
                viewMode === 'list'
                  ? 'bg-white text-blue-600 shadow-2xs'
                  : 'text-slate-500 hover:text-slate-800'
              }`}
              title="List View"
            >
              <List className="w-3.5 h-3.5" />
              <span className="hidden sm:inline">List</span>
            </button>
          </div>
        </div>
      </div>

      {/* Results summary strip */}
      <div className="flex items-center justify-between text-[11px] text-slate-500 pt-1 border-t border-slate-100">
        <div>
          Showing <span className="font-bold text-slate-800">{totalResults}</span> {totalResults === 1 ? 'session' : 'sessions'}
        </div>
        {hasActiveFilters && (
          <span className="text-blue-600 font-semibold">
            Filtered view engaged
          </span>
        )}
      </div>
    </div>
  );
};

export default SessionToolbar;
