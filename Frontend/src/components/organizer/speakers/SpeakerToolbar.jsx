  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200/80 p-3.5 shadow-2xs space-y-3">
      <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-3">
        ...
        <div className="relative flex-1 min-w-[260px]">
          ...
          <input
            ...
            placeholder="Search by name, company, designation or session..."
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="relative">
            <select
              ...
              className="appearance-none pl-3 pr-8 py-2 text-xs font-semibold text-slate-700 bg-slate-50/70 border border-slate-200 rounded-xl hover:bg-slate-100/60 focus:outline-none focus:ring-2 focus:ring-blue-600/20 focus:border-blue-600 transition-all cursor-pointer max-w-[170px] truncate"
            >
              <option value="all">All Events</option>
              <option value="Global Tech Leadership Summit 2026">Global Tech Leadership</option>
              <option value="AI & Cloud Innovation Conference">AI & Cloud Innovation</option>
              <option value="FinTech Future Forum">FinTech Future Forum</option>
              {events.map((ev, i) => (
                <option key={i} value={ev.title || ev}>
                  {ev.title || ev}
                </option>
              ))}
            </select>
          ...
          <div className="relative">
            <select
              ...
              className="appearance-none pl-3 pr-8 py-2 text-xs font-semibold text-slate-700 bg-slate-50/70 border border-slate-200 rounded-xl hover:bg-slate-100/60 focus:outline-none focus:ring-2 focus:ring-blue-600/20 focus:border-blue-600 transition-all cursor-pointer max-w-[150px] truncate"
            >
              <option value="all">All Sessions</option>
              {sessions.map((ses, i) => (
                <option key={i} value={ses.title || ses}>
                  {ses.title || ses}
                </option>
              ))}
            </select>
      ...
      <div className="flex items-center justify-between text-[11px] text-slate-500 pt-1 border-t border-slate-100">
        <div>
          Showing <span className="font-bold text-slate-800">{totalResults}</span> {totalResults === 1 ? 'speaker' : 'speakers'}
        </div>
        {hasActiveFilters && (
          <span className="text-blue-600 font-semibold">
            Filtered view engaged
          </span>
        )}
      </div>
    </div>
